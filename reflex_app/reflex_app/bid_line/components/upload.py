"""PDF Upload Component for Bid Line Analyzer.
...
def upload_component() -> rx.Component:
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.text(
                "Upload your bid line packet to analyze scheduling metrics",
        ...
        rx.cond(
            rx.selected_files("bid_line_upload").length() > 0,
            rx.box(
                rx.vstack(
                    rx.hstack(
                    ...
                    rx.foreach(
                        rx.selected_files("bid_line_upload"),
                        lambda file: rx.text(
                            file,
                            size="2",
        ...
        rx.button(
            ...
            on_click=BidLineState.handle_upload(
                rx.upload_files(upload_id="bid_line_upload")
            ),
            size="3",
            disabled=BidLineState.is_processing,
            width="100%",
            max_width="600px",
            cursor="pointer",
        ),
        ...
        rx.cond(
            ...
            rx.box(
                rx.vstack(
                    ...
                    rx.progress(
                        value=BidLineState.processing_progress,
                        max=100,
                        width="100%",
                        height="10px",
                        color_scheme="blue",
                    ...
                    rx.hstack(
                        rx.text(
                            BidLineState.processing_message,
                            size="2",
                            color=Colors.gray_600,
                        ),
                        rx.text(
                            BidLineState.processing_progress.to(str) + "%",
                            size="2",
                            font_weight=Typography.weight_bold,
        ...
        # Success message
        rx.cond(
            BidLineState.uploaded_file_name != "",
            rx.box(
                rx.vstack(
                    rx.hstack(
                    ...
                    rx.text(
                        "Filename: " + BidLineState.uploaded_file_name,
                        size="2",
                        color=Colors.gray_700,
                        font_family=Typography.font_mono,
                    ),
